...
export function Like({ likedByUser, likes, postId, sessionUserId }: LikeProps) {
  ...
  return (
    <fetcher.Form action="/resources/like" method="post">
      ...
      >
        <Star
          className={`w-4 h-4 group-hover:text-yellow-500 fill-current ${
            optimisticLikedByUser ? "text-yellow-400" : "text-yellow-400"
          }`}
        />
        <span
          className={`ml-2 text-sm group-hover:text-yellow-500 ${
            optimisticLikedByUser ? "text-yellow-400" : "text-yellow-400"
          }`}
        >
          {optimisticLikes}
